...
#Do the git-ing
cmd='git commit -a -m "Commit before run STM" > /dev/null'
print(cmd)
...
for L in xrange(16):
	...
	task_file.write('bash STM_'+str(vnum)+'.qsub\n')
	vmap_file.write(str(vnum)+'\t'+str(L)+'\n')
	task_file.write('bash STM_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('STM.sbatch.template','r').read()
...
open('STM.sbatch','w').write(contents)
