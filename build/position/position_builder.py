...
#Do the git-ing
cmd='git commit -a -m "Commit before run position" > /dev/null'
print(cmd)
...
open('position.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('position.task','w')
template_file='position.template'
...
for L in xrange(16):
	...
	task_file.write('bash position_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('position.sbatch.template','r').read()
...
open('position.sbatch','w').write(contents)
