...
#Do the git-ing
cmd='git commit -a -m "Commit before run 2_force" > /dev/null'
print(cmd)
...
open('2_force.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('2_force.task','w')
template_file='2_force.template'
...
for L in xrange(32):
	...
	task_file.write('bash 2_force_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('2_force.sbatch.template','r').read()
...
open('2_force.sbatch','w').write(contents)
