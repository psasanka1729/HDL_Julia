...
#Do the git-ing
cmd='git commit -a -m "Commit before run 4_force" > /dev/null'
print(cmd)
...
open('4_force.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('4_force.task','w')
template_file='4_force.template'
...
for L in xrange(32):
	...
	task_file.write('bash 4_force_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('4_force.sbatch.template','r').read()
...
open('4_force.sbatch','w').write(contents)
