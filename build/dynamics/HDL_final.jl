...
psi = zeros(ComplexF64,(1,2^(1+Nx*Ny_max*6)));
Hilbert = collect(it);
for i = 1:2^(1+Nx*Ny_max*6)
	if Hilbert[i][H_position] == 1
		psi[i] = 1
	end
end
...
# The coupled ODEs. =#
dxdt(t,x,p) = p/m_e;
dpdt(t,x,p) = real(-k_potential*(x-x_0/2)-(Psi'*dHamiltonian(x)*Psi)[1]);
...
while t<t_end
    ...
    M     = -1im*Hamiltonian_variable(x)*dt/hbar
    Psi_1 =  M*Psi
    Psi_2 = (1/2)*M*Psi_1
    Psi_3 = (1/3)*M*Psi_2
    Psi_4 = (1/4)*M*Psi_3
    global Psi = Psi + Psi_1 + Psi_2 + Psi_3 + Psi_4
    global Psi = Psi/norm(Psi) 
end
