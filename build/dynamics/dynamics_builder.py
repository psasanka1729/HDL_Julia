...
#Do the git-ing
cmd='git commit -a -m "Commit before run dynamics" > /dev/null'
print(cmd)
...
open('dynamics.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('dynamics.task','w')
template_file='dynamics.template'
...
for L in xrange(16):
	...
	task_file.write('bash dynamics_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('dynamics.sbatch.template','r').read()
...
open('dynamics.sbatch','w').write(contents)
