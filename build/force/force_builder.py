...
#Do the git-ing
cmd='git commit -a -m "Commit before run force" > /dev/null'
print(cmd)
...
for L in xrange(32):
	...
	task_file.write('bash force_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('force.sbatch.template','r').read()
...
open('force.sbatch','w').write(contents)
