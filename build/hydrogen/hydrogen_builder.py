...
#Do the git-ing
cmd='git commit -a -m "Commit before run hydrogen" > /dev/null'
print(cmd)
...
open('hydrogen.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('hydrogen.task','w')
template_file='hydrogen.template'
template_contents=open(template_file,'r').read()

vnum=0
ks = [8,8.5,9,9.5,10,10.5,11,11.5,12,12.2,13.4,13.6,13.8,14,14.2,14.4,14.6,14.8,15,15.2,15.4,15.6,15.8,16]
for L in xrange(16):
...
contents=open('hydrogen.sbatch.template','r').read()
...
open('hydrogen.sbatch','w').write(contents)
