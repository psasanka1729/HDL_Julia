...
function Hamiltonian_variable(x1)
    #x1 = real(x1)
    H_x = zeros(ComplexF64,2^(1+Nx*Ny_max*6),2^(1+Nx*Ny_max*6));
...
function dHamiltonian(x)
    #x = real(x)
    local dHx = zeros(ComplexF64,2^(1+6*Nx*Ny_max),2^(1+6*Nx*Ny_max));
...
X0 = LinRange(0.6,2,6)*10^(-10)
Force = []
F(x1,Psi1) = -real((Psi1'*dHamiltonian(x1)*Psi1)[1]-dVdx(x1))
for xs in X0
