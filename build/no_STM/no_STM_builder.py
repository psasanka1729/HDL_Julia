...
#Do the git-ing
cmd='git commit -a -m "Commit before run no_STM" > /dev/null'
print(cmd)
...
for L in xrange(16):
	...
	task_file.write('bash no_STM_'+str(vnum)+'.qsub\n')
	vmap_file.write(str(vnum)+'\t'+str(L)+'\n')
	task_file.write('bash no_STM_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('no_STM.sbatch.template','r').read()
...
open('no_STM.sbatch','w').write(contents)
