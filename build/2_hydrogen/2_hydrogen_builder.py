...
#Do the git-ing
cmd='git commit -a -m "Commit before run 2_hydrogen" > /dev/null'
print(cmd)
...
open('2_hydrogen.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('2_hydrogen.task','w')
template_file='2_hydrogen.template'
...
contents=open('2_hydrogen.sbatch.template','r').read()
...
open('2_hydrogen.sbatch','w').write(contents)
